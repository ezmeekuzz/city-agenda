$(document).ready(function () {
    var table = $('#ticketmasterlist').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": "/organizer/ticketmasterlist/getData",
            "type": "GET"
        },
        "columns": [
            { "data": "eventname" },
    ...
    $(document).on('click', '.delete-btn', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/organizer/ticketmasterlist/delete/' + id,
                    method: 'DELETE',
    ...
    function createNoTicketsMessage(modalBody, event_id) {
        ...
        document.getElementById('submitTicketForm').addEventListener('click', function() {
            ...
            window.location.href = `/organizer/publish-event/${eventId}`;
        });
    }
    $(document).on('click', '#submitTicketForm', function () {
        ...
        if (isValid) {
            ...
            $.ajax({
                url: '/organizer/ticketmasterlist/update',  // Update the URL to the correct route for handling updates
                method: 'POST',
                data: formData,
                success: function (response) {
                    if (response.success) {
                        Swal.fire({
                            ...
                            window.location.href = "/organizer/publish-event/" + $('#event_id').val()
                        });
                    } else {
                        Swal.fire({
                            icon: 'error',
